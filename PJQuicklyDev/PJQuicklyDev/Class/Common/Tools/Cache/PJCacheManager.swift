...
public struct PJCacheManager {
    ...
    //保存大对象(只要是遵循HandyJSON协议的都可以)
    public static func saveBigObject<T: HandyJSON>(key:String, value: T, forPath: String) {
    ...
    public static func getBigObject<T: HandyJSON>(key:String, returnClassType : T) -> T? {
        do {
            let bigObjectPath = self.shared.documnetPath + self.bigObject + key.md5()
    ...
    public static func getObject<T: HandyJSON>(key:String, returnClassType : T) -> T? {
